...
import { PrismaToAPIVideo, APIVideo, APICollection} from "helpers/api";
import {
  YouTubeDataChannel,
  YouTubeDataVideo,
...
export default async function handler(
  ...
  if (req.method === "GET") {
    ...
    if (collection) {
      const videos: VideosOnCollections = await prisma.collection.findMany({
        where: { id: `${collection.videos[].videoID}` },
      });
